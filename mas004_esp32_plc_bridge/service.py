...
from mas004_esp32_plc_bridge.config import Settings, DEFAULT_CFG_PATH


def probe(cfg: Settings) -> tuple[bool, str]:
    if cfg.watchdog_host:
        try:
            if ping(cfg.watchdog_host, timeout=cfg.timeout_s, unit="s") is None:
                return False, f"watchdog ping failed: {cfg.watchdog_host}"
        except Exception as exc:
            return False, f"watchdog ping error: {repr(exc)}"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="MAS-004 ESP32 bridge service")
